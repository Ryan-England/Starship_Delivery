...
public class JsonManager : MonoBehaviour
{
    ...
    {
        var json_Object = new JSONObject(json);
        if (json_Object.list[0].type != JSONObject.Type.Array) //ERROR - Resulting object is never an array - needs correction - make dummy and check resulting type of json_Object via switch statement messages
        {
            Debug.LogError("Json file is not an array");
            return;
        }
